...
void setup() {
    ...
    if (!WiFi.softAP(ssid, pass)) {  // You can set a password if desired
        Serial.println("Failed to create access point");
        while (true);  // Stop if creating AP fails
    }
...
void loop() {
    // Check if any devices are connected to the AP
    int numStations = WiFi.softAPgetStationNum();
    static int lastNumStations = 0;

    // Only print when the connection status changes
    if (numStations != lastNumStations) {
        lastNumStations = numStations;
    ...
    // Read and process client commands if a device is connected
    if (client.connected() && client.available() > 0) {
        ...
        // Execute command only if a new command is received
